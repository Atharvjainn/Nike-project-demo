...
const Signin = () => {
  ...
  return (
    <section className= 'w-full h-[96vh] pt-3 padding-x flex items-center justify-center font-montserrat'>
        <div className={`max-w-[1440px] mx-auto my-0 flex justify-center items-center
            ${ishovered ? "xl:w-[40%]" : "xl:w-full"} w-full  bg-white sm:bg-gray-800 transition-all ease-in-out duration-500 h-[80%] rounded-2xl shadow-3xl `}>
            <div className=' bg-black/85 flex flex-col py-12 px-11 rounded-2xl shadow-3xl border-4 border-orange-400'
            onMouseEnter={() => setIshovered(true)}
            onMouseLeave={() => setIshovered(false)}
            >
    <form  className='flex flex-col text-coral-red items-center gap-4' onSubmit={onclickhandler}
        ref={ref}
    >
        <div>
            <h1 className='text-4xl font-bold font-montserrat mb-3   '>SIGN UP</h1>
        </div>
        ...
        <div className='min-w-[263.93px] text-center'>
            <p >Have an account? <Link className='text-blue-500 font-bold' to="/Login">Login</Link></p>
        </div>
